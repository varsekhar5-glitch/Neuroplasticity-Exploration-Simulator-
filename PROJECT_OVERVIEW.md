...
- **`research/literature.csv`** — 55 evidence rows at that point, 104 after the third pass below (one per study × outcome), each with population, n, effect size, 95% CI, p-value, confidence rating, disease match (AD / PD / MCI / healthy), and the model parameter it feeds. Every quantitative value was verified against the fetched abstract (PMIDs/DOIs recorded).
...
- The app **loads `benchmarks.json` at boot**, derives the validation panel's trial endpoints from it, and auto-lists every source row in a machine-readable table in Science & Model Notes.
...
## Third calibration pass (2026-09-11): 49 new verified sources, five assumptions removed
Two parallel literature searches (Alzheimer's/APOE and Parkinson's/dopamine) grew `research/literature.csv` from 55 to 104 rows, every number verified against the fetched abstract. The new data overturned five model assumptions, each now documented inline in `app.js` and in the Science & Model Notes "Calibration pass 3" section:

1. **Lifestyle amyloid clearance removed.** Omega-3 left CSF Aβ42 unchanged (Tofiq 2021, OmegAD CSF); PreventE4 (Yassine 2026, n=365) found no brain-volume or cognition difference at 24 months; 15 years of AIBL activity data show no amyloid effect (Slee 2024, n=731). Plaque now accrues at the Villemagne 2013 / Bollack 2024 rate (~4 centiloids/yr) and nothing on the sliders removes it.
2. **The ε4 × omega-3 boost was backwards** (Arellanes 2020: CSF EPA rose 3× more in *non*-carriers; PreventE4: no ε4 interaction). Removed.
3. **Exercise builds resilience, not clearance** (Rabin 2019: activity blunts amyloid's effect on cognitive decline, β 0.03 [0.02, 0.05]). Modeled as up to 35% attenuation of the plaque→cognition penalty.
4. **APOE4 atrophy penalty was ~3× too strong.** Refit to ~1.1 %/yr extra per copy (Schuff 2009 ADNI −1.4 %/yr; Cai 2026 ~−1 %/yr).
5. **Meditation does not rescue dopamine.** No PD dopamine-imaging data exist; Kjaer 2002's +65% release was acute, in healthy meditators, in the ventral striatum. Meditation now acts only on a symptomatic UPDRS-III offset capped at 0.6× exercise (Cochrane 2024 network meta; Kwok 2025; Advocat 2016 null).

Three magnitudes moved to better-matched populations: exercise → CRP −4 → −15 (Hong 2026 older-adult network meta, SMD −0.73); exercise → BDNF in Parkinson's mode uses the PD-specific Kaagman 2024 value (SMD 1.2 → 24); and the dopamine decline is now two-slope (−11.2% year 1, −17% cumulative at 24 months, Simuni 2018 PPMI n=423) instead of linear. A new **UPDRS-III motor series** (baseline 20.9, +2.4 pts/yr untreated per Holden 2018; exercise −4.2 pts per Park-in-Shape 2019) appears in Parkinson's mode with its own gauge, scoreboard row and validation row.

**The previously open issue is closed.** The structural constants (atrophy, inflammation penalty, brain→cognition mapping) were grid-fitted by least squares so the untreated ε4×1 run reproduces the published 24-month endpoints: hippocampus −9.3% (target −9.3), MMSE −6.5 (target −6.6), APOE spread ±3.5% per copy. Before the refit that run lost 28% of hippocampal volume and 14 MMSE points. The validation panel now carries five benchmark scenarios instead of two: untreated, exercise (EXERT 2025 aMCI, replacing the healthy-adult Erickson 2011), omega-3 (MAPT, PreventE4, OmegAD: all null, so the endpoint equals untreated and the residual displays the model's over-prediction), CST (Woods 2023), meditation (Shi 2025) and multi-domain (US POINTER 2025 + J-MINT). A benchmark with no published endpoint for a metric omits that row rather than inventing one.
...
- **Validation & Metrics panel** — reads the current sliders, matches them to the closest published trial scenario (Barnes & Fox 2009 + Han 2000 for untreated AD; EXERT 2025 + DAPA 2018 for exercise-only; MAPT 2017 + PreventE4 2026 + OmegAD 2006 for omega-3-only; Woods 2023 for CST; Shi 2025 for meditation; US POINTER 2025 + J-MINT for multi-domain), and shows simulated vs. trial 24-month endpoints for hippocampal volume and MMSE with per-metric **residuals** and a normalized model–trial agreement score. With Parkinson's mode on, a dopamine row benchmarks the simulated decline against Simuni 2018 PPMI serial DAT imaging (−17% at 24 months) and a UPDRS-III row against Holden 2018 (+2.4 pts/yr) or Park-in-Shape 2019 (−4.2 pts with exercise).
...
### Pathology Lab
- **Parkinson's dopamine circuit** — the substantia nigra → striatum (nigrostriatal) pathway with dopamine particles flowing along the axons. Raise *neuron loss* and dopamine falls, particles thin, and the whole circuit develops a tremor; add *exercise + mind-body training* to partly restore it (exercise raises striatal dopamine release, Sacheli 2019; meditation eases motor scores without evidence of a dopamine effect, Kwok 2025). Live dopamine / motor / tremor gauges.
...
- **Calibration pass 3 section** — the five removed assumptions, the recalibrated magnitudes, and 21 new linked source cards (PubMed/PMC/JAMA/Frontiers) covering PPMI progression, Park-in-Shape, the Cochrane PD exercise network meta-analysis, EXERT, DAPA, MAPT, PreventE4, OmegAD CSF, AIBL amyloid, Rabin 2019, US POINTER, and more.
- **Literature base table (auto-generated)** — all 104 machine-readable source rows from `research/literature.csv`, listed with study, design, outcome, effect size, and the model parameter each feeds; spans Alzheimer's, Parkinson's/dopamine, APOE4, and all four interventions.
